...
async function processBlocks() {
    ...
    chrome.runtime.sendMessage({action: "initializeProgressBar", length: fullblocks.length});
...
globalWordFrequency = new Map(globalArrayFrequency);
    ...
    chrome.runtime.sendMessage({action: "wordCounterUpdate", globalWordFrequency: globalWordFrequency });
    
    if (i % 3 == 0){
        chrome.runtime.sendMessage({action: "wordCloudUpdate", globalWordFrequency: globalWordFrequency });

    }
    

    chrome.runtime.sendMessage({action: "progessBarUpdate", width: ((i + 1) / fullblocks.length) * 100 + '%', iter: i});


  }

  chrome.runtime.sendMessage({action: "finishProgress"});
